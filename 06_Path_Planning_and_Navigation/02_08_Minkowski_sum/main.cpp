...
// ***TODO: Check for duplicate coordinates inside a 2D vector and delete
// them*** //
vector<vector<int>> delete_duplicate(vector<vector<int>> C) {
  ...
  for (size_t i = 0; i < C.size() - 1; i++) {
    // Check if its a duplicated coordinate
    if (C[i] != C[i + 1]) {
      Cn.push_back(C[i]);
    }
  }
...
vector<vector<int>> minkowski_sum(vector<vector<int>> A,
                                  vector<vector<int>> B) {
  vector<vector<int>> C;
